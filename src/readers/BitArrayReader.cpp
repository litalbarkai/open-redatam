...
bitset<32> BitArrayReader::CreateMask(size_t size)
{
    bitset<32> mask = 0;
...
void BitArrayReader::ParseBits(vector<uint32_t>* results, uint32_t data)
{
    bitset<32> remainderMask = CreateMask(m_remainderSize);
    bitset<32> bits = data;
    m_data = m_remainder |
